...
class BenchmarkConfig:
    ...
    matlab_startup_options: str = "-nojvm"
...
def run_matlab_sidecar_engine(config: BenchmarkConfig, bundle_json_path: Path, job_dir: Path, helper_m: Path) -> pd.DataFrame:
    ...
    try:
        ...
        eng.analyzir_arirls_batch(nargout=0)
    except Exception as exc:
...
def run_matlab_sidecar(config: BenchmarkConfig, bundle_json_path: Path, job_dir: Path) -> pd.DataFrame:
    ...
    if use_engine:
        try:
            ...
            if config.use_matlab_engine and not config.prefer_matlab_engine:
                raise
                ...
                if len(matlab_df) > 0:
                    ...
                    if len(shifted_matlab) > 0:
                        ...
                        for pipeline_label, subgrp in shifted_matlab.groupby("pipeline_label"):
                            ...
                            auc_scale = float(np.trapz(basis, epoch_times))
                            tmp = subgrp.copy()
...
def build_config_from_args(args: argparse.Namespace) -> BenchmarkConfig:
    config = BenchmarkConfig(
        ...
        use_matlab_engine=args.use_matlab_engine,
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--matlab-startup-options", default="-nojvm")
    ...
    with ProcessPoolExecutor(max_workers=config.n_workers) as executor:
        ...
        for future in tqdm(
            ...
            try:
                ...
                run_results.append(
                    {"subject": subject, "file_label": file_label, "status": "crashed-submit", "error": str(exc)}
                )
