...
# Overnight batch goal:
#   - keep the hard case (Subj94)
#   - add at least 4 more available subjects automatically
manual_subject_names = [
    "Subj86", "Subj91", "Subj92", "Subj94", "Subj95", "Subj96",
    "Subj97", "Subj98", "Subj99", "Subj100", "Subj101",
    "Subj102", "Subj103", "Subj104",
]
auto_additional_subject_count = 4
auto_select_additional_subjects = False

file_labels = ["no_hrf", "hrf_20"]
output_prefix = "batch_true_injected_low_amp_v4"

# PFIR is slow and was not trustworthy in the last probe, so leave it off by
# default for the overnight sanity run. Turn this on later if needed.
include_fir_pipeline = False
...
def resolve_subject_names():
    ...
    for subject in available_subjects:
        if subject in resolved_subjects:
            continue
        resolved_subjects.append(subject)
        if len(resolved_subjects) >= len(manual_subject_names) + auto_additional_subject_count:
            break
...
# -----------------------------------------------------------------------------
# Error logging / intermediate saving
# -----------------------------------------------------------------------------
...
def build_short_regressor_signal(raw_hb, chromophore, ss_method):
    ...
    if ss_method == "pca":
        ...
        return data_matrix @ first_component_weights
...
def build_design_matrix_for_chromophore(raw_hb, chromophore, model_type, ss_method):
...
def run_canonical_glm_for_channel(raw_hb, channel_name, pipeline_label, file_label, design_matrix, target_pair_names):
...
def run_fir_glm_for_channel(raw_hb, channel_name, pipeline_label, file_label, design_matrix, target_pair_names):
    ...
    for file_label_value in file_labels:
        ...
        for pipeline_label in pipeline_labels:
            ...
            if pipeline_spec["estimation_method"] == "canonical_glm":
                ...
                try:
                    for chromophore in ["hbo", "hbr"]:
                        design_matrix_cache[chromophore] = build_design_matrix_for_chromophore(
                            raw_hb=raw_hb,
                            chromophore=chromophore,
                    ...
                    error_rows.append(make_error_row(subject_name, file_label, pipeline_label, "canonical_design_matrix", exc))
                    continue
                ...
                for channel_name in all_available_long_channel_names:
                    ...
                    try:
                        canonical_metrics = run_canonical_glm_for_channel(
                            ...
                            pipeline_label=pipeline_label,
                            file_label=file_label,
                            design_matrix=design_matrix_cache[chromophore],
                ...
                try:
                    ...
                    error_rows.append(make_error_row(subject_name, file_label, pipeline_label, "fir_design_matrix", exc))
                    continue
...
def build_null_false_positive_summary_df(canonical_channel_df):
...
def save_epoch_trace_figure(epoch_plot_df):
    ...
    epoch_figure = px.line(
        ...
        title="Pre-estimation event-locked traces on true target channels (Pref only, context)",
    )
...
def save_target_minus_non_target_figure(target_vs_nontarget_summary_df):
    ...
    separation_figure.update_layout(width=1600, height=850)
...
def save_channel_distribution_figure(channel_df, value_column, score_label, figure_suffix):
    ...
    distribution_figure.update_layout(width=1600, height=900)
...
def save_outputs(truth_summary_rows, all_quality_tables, all_channel_availability_tables,
    ...
    null_false_positive_summary_df = build_null_false_positive_summary_df(canonical_channel_df)
