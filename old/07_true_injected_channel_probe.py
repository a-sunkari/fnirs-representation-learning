...
# Small probe first.
# Include Subj94 because the subject sheet flags it as poor SNR.
subject_names_to_run = ["Subj94", "Subj100", "Subj101", "Subj102"]
...
def read_measurement_data_type_labels(snirf_file_path):
    ...
    with h5py.File(snirf_file_path, "r") as h5_file:
        ...
        # Prefer data1 if present, otherwise first data* group.
...
def build_subject_probe(subject_name_value):
    ...
    # Use the true injected channels from the semisynthetic labels, not a geometry heuristic.
    target_pair_names = get_true_injected_pair_names(reference_file_path, reference_raw_cw)
    selected_channel_names = get_selected_channel_names_from_pairs(target_pair_names)
    ...
    for file_label in file_labels:
        ...
        for pipeline_label in pipeline_labels:
            ...
            pair_quality_table["is_true_target"] = pair_quality_table["pair_name"].isin(target_pair_names)
            ...
            if pipeline_spec["estimation_method"] == "canonical_glm":
                for channel_name in available_channel_names:
                    ...
                    canonical_metrics["is_true_target"] = canonical_metrics["pair_name"] in target_pair_names
                ...
                if len(block_average_channel_df) > 0:
                    block_average_channel_df["is_true_target"] = block_average_channel_df["pair_name"].isin(target_pair_names)
                ...
                for channel_name in available_channel_names:
                    ...
                    if len(fir_channel_df) > 0:
                        fir_channel_df["is_true_target"] = fir_channel_df["pair_name"].isin(target_pair_names)
...
def main():
    ...
    truth_summary_df.to_csv(output_tables_dir / "true_injected_low_amp_truth_summary.csv", index=False)
    quality_summary_df.to_csv(output_tables_dir / "true_injected_low_amp_quality_summary.csv", index=False)
    canonical_channel_df.to_csv(output_tables_dir / "true_injected_low_amp_canonical_channel_metrics.csv", index=False)
    block_average_channel_df.to_csv(output_tables_dir / "true_injected_low_amp_block_average_channel_metrics.csv", index=False)
    if len(fir_channel_df) > 0:
        fir_channel_df.to_csv(output_tables_dir / "true_injected_low_amp_fir_channel_metrics.csv", index=False)

    roi_summary_tables = []
    if len(canonical_channel_df) > 0:
        canonical_roi_df = (
            canonical_channel_df
            .groupby(["subject", "file_label", "amplitude_value", "pipeline_label", "chromophore", "is_true_target"], as_index=False)
            .agg(
    ...
    roi_score_df.to_csv(output_tables_dir / "true_injected_low_amp_roi_scores.csv", index=False)

    if len(canonical_channel_df) > 0:
        channel_separation_df = (
            canonical_channel_df
            .groupby(["subject", "file_label", "pipeline_label", "chromophore", "is_true_target"], as_index=False)
            .agg(mean_beta=("beta", "mean"))
        )
        channel_separation_wide_df = (
            channel_separation_df
            .pivot_table(
                index=["subject", "file_label", "pipeline_label", "chromophore"],
                columns="is_true_target",
                values="mean_beta",
            )
            .reset_index()
            .rename(columns={False: "mean_non_target_beta", True: "mean_target_beta"})
        )
        if "mean_target_beta" not in channel_separation_wide_df.columns:
            channel_separation_wide_df["mean_target_beta"] = np.nan
        if "mean_non_target_beta" not in channel_separation_wide_df.columns:
            channel_separation_wide_df["mean_non_target_beta"] = np.nan
        channel_separation_wide_df["target_minus_non_target_beta"] = (
            channel_separation_wide_df["mean_target_beta"] - channel_separation_wide_df["mean_non_target_beta"]
        )
        channel_separation_wide_df.to_csv(output_tables_dir / "true_injected_low_amp_target_vs_nontarget_summary.csv", index=False)

    if len(fir_channel_df) > 0:
        fir_shape_rows = []
        grouped_fir_df = fir_channel_df.groupby(["subject", "file_label", "pipeline_label", "chromophore", "is_true_target"])
        for (subject_name_value, file_label_value, pipeline_label_value, chromophore, is_true_target), group_df in grouped_fir_df:
        ...
        fir_shape_summary_df.to_csv(output_tables_dir / "true_injected_low_amp_fir_shape_summary.csv", index=False)
    ...
    if len(epoch_plot_df) > 0:
        epoch_figure = px.line(
            ...
            title="True injected-channel low-amplitude probe: event-locked traces",
        )
        epoch_figure.update_layout(width=1200, height=800)
        epoch_figure.write_html(output_figures_dir / "true_injected_low_amp_epoch_traces.html")
    ...
    if len(roi_score_df) > 0:
        roi_figure = px.line(
            ...
            line_dash="is_true_target",
            markers=True,
            title="True injected-channel low-amplitude probe: target vs non-target ROI scores",
        )
        roi_figure.update_layout(width=1400, height=750)
        roi_figure.write_html(output_figures_dir / "true_injected_low_amp_roi_scores.html")

    print("Saved true injected-channel low-amplitude probe outputs.")
