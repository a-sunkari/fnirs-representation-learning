...
subject_names_to_run = ["Subj94"]
file_labels = ["null", "hrf_20"]

# Keep outputs separate from earlier multi-subject runs.
output_prefix = "subj94_true_injected_low_amp"
...
def build_subject_probe(subject_name_value):
    ...
    for file_label in file_labels:
        ...
        for pipeline_label in pipeline_labels:
            ...
            available_pair_names = sorted(set([channel_name.split(" ")[0] for channel_name in available_channel_names]))
            available_non_target_pair_names = [
                pair_name for pair_name in non_target_pair_names
                if (pair_name + " hbo") in raw_hb.ch_names and
                   (pair_name + " hbr") in raw_hb.ch_names and
                   (pair_name + " hbo") not in raw_hb.info["bads"] and
                   (pair_name + " hbr") not in raw_hb.info["bads"]
            ]
            ...
            channel_availability_rows.append({
                ...
                "n_available_true_target_pairs": len(available_pair_names),
                ...
                "target_pair_retention_fraction": (len(available_pair_names) / len(target_pair_names)) if len(target_pair_names) > 0 else np.nan,
...
def main():
    ...
    if len(canonical_channel_df) > 0:
        ...
        canonical_separation_wide_df = (
            ...
            .reset_index()
            .rename(columns={False: "mean_non_target_score", True: "mean_target_score"})
        )
    ...
    if len(canonical_channel_df) > 0 and "p_value" in canonical_channel_df.columns:
        canonical_null_df = canonical_channel_df.loc[canonical_channel_df["file_label"] == "null"].copy()
        if len(canonical_null_df) > 0:
            ...
            null_false_positive_summary_df = (
                canonical_null_df
                .groupby(["subject", "pipeline_label", "chromophore", "is_true_target"], as_index=False)
                .agg(
    ...
    if len(epoch_plot_df) > 0:
        ...
        epoch_figure.write_html(output_figures_dir / f"{output_prefix}_epoch_traces.html")
    ...
    if len(roi_score_df) > 0:
        ...
        roi_figure.write_html(output_figures_dir / f"{output_prefix}_roi_scores.html")
